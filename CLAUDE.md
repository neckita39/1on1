...
## Мобильная вёрстка (iPhone)

Брейкпоинт один — 768px (Tailwind `md`), он же в мобильном слое `index.css`.

- **< 768px**: сайдбара нет, вместо него фиксированный таб-бар снизу (Команда · Встреча ·
  Дейли · Важное · Ещё) + шторка «Ещё» с командными заметками, историей, часами и выходом.
- **≥ 768px**: всё как было — сайдбар слева, двухколоночные экраны.
- `useIsMobile()` из `src/ui` — там, где мобильная и десктопная разметка расходятся по DOM
  (шторка вместо модалки, стрелки вместо drag & drop, порядок блоков карточки сотрудника).
- Правила мобильного слоя (`index.css`, `@media (max-width: 767px)`):
  `.input-spec/select` — 16px (иначе iOS зумит страницу при фокусе), `.btn-spec`/`.tap`/`.tap-sm`/
  `.check-hit` — минимальные размеры зон нажатия, `.mobile-carousel` — ряд метрик в карусель со
  снапом, `.mobile-scroll-x` — полосы вкладок/фильтров, `.cat-select` — категория пункта в пилюлю.
- `@media (hover: none) .touch-visible` — показывает действия, спрятанные под ховером
  (удаление встречи в истории, «Изменить» в заметках).
- Safe area: `viewport-fit=cover` в `index.html`, переменные `--tabbar`/`--safe-b` в `:root`;
  на них завязаны нижний отступ `main`, тост и виджет дейли.
- Порядок 1-1 в повестке на тач-экране меняется кнопками (`MoveButton`/`moveAgendaItem`
  в `components/AgendaList.tsx`) — события HTML5 drag & drop на тач не приходят.
